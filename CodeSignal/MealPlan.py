...
# You've created a meal plan for the next few days, and prepared a list of products that you'll
# need as ingredients for each day's meal. There are many shops around you that sell the products
# you're looking for, but you only have time to visit one or two stores each day.

# Given the following information, your task is to find the minimum cost you'll need to spend on
# each meal:

# cntProducts - an integer representing the total number of products you'll be using in all of your
# meals;

# quantities - a rectangular matrix of integers, where quantities[i][j] represents the amount of
# product j available in shop i;

# costs - a rectangular matrix of integers, where costs[i][j] represents the cost of buying product
# j from shop i;

# meals - a rectangular matrix of integers, where meals[m][j] represents the amount of product j
# required to make the mth meal.

# Return an array of length meals.length representing the minimum cost of each meal (assuming you
# can only visit up to two shops each day).

# NOTE:

# You only like to use fresh ingredients, so you'll need to buy new products from the shops each
# day (you may not use leftovers from a previous day).
# Each store re-stocks their merchandise every night, so the amounts in the quantities array are
# available each day.
# It's guaranteed that it will always be possible to buy all the products needed for each meal by
# visiting only one or two shops.
...
#          [2, 2],
#          [3, 4]]
# choosingShops(cntProducts, quantities, costs, meals) = [3, 8, 19].
...
# Return an array of integers of length meals.length, where the ith element represents the minimum
# cost for buying the products to cook the ith meal, visiting only one or two shops.
